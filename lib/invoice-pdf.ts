...
import { formatDate } from './estimate-pdf'
...
export async function generateInvoicePDF(invoice: InvoiceData): Promise<Buffer> {
  ...
  const hasExtras  = Number(invoice.additionalFees) > 0 || Number(invoice.taxRate) > 0
  const totalsHtml = hasExtras ? `
    <div class="total-row"><span>Subtotal</span><span>$${Number(invoice.subtotal).toFixed(2)}</span></div>
    ${Number(invoice.additionalFees) > 0 ? `<div class="total-row"><span>Additional Fees</span><span>$${Number(invoice.additionalFees).toFixed(2)}</span></div>` : ''}
    ${Number(invoice.taxRate) > 0 ? `<div class="total-row"><span>Tax (${invoice.taxRate}%)</span><span>$${Number(invoice.taxAmount).toFixed(2)}</span></div>` : ''}
  ` : ''
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .invoice-word { position: absolute; top: 155px; right: 0; font-size: 32px; font-weight: 700; color: #4b3fa0; opacity: 0.5; letter-spacing: 3px; }
...
<div class="page">
  ...
  <div class="bill-section">
    ...
    <div style="text-align:right">
      ...
      <div class="inv-meta">
        ...
        ${invoice.periodFrom && invoice.periodTo ? `Period: ${formatDate(invoice.periodFrom)} – ${formatDate(invoice.periodTo)}<br/>` : ''}
        Status: <strong style="color:${statusColor}">${invoice.status.toUpperCase()}</strong>
        ${invoice.paidAt ? `<br/>Paid: ${formatDate(invoice.paidAt)}` : ''}
      </div>
    </div>
  </div>
  <table>
  ...
  ${invoice.notes ? `<div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:7px;padding:10px 14px;margin-bottom:18px;font-size:11px;color:#4b5563;line-height:1.7"><strong style="font-size:10px;text-transform:uppercase;letter-spacing:0.8px;color:#9ca3af;">Notes</strong><br/>${invoice.notes}</div>` : ''}
